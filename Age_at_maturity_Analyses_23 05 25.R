...
Out[[1]] + theme(axis.title.x = element_text(size = 14, color = "black", family = "serif")) #+ scale_color_manual(name = "", labels = c("M" = "Male", "F" = "Female"), values = c("#000000", "#666666")) 
#
...
(S_WWwCI  <- ggplot()+
    ...
    geom_line(data = model_S_WW, aes(binSL, predWW, color = "Predict"))+
...
#
(S_WWwCI_eq <- S_WWwCI +
    annotate("text", x = 12, y = 78, label = "log(WW) = 0.06 * SL - 0.37",
...
(S_SWwCI  <- ggplot()+
    ...
    geom_line(data = model_S_SW, aes(binSL, predSW, color = "Predict"))+
...
#
(S_SWwCI_eq <- S_SWwCI +
    annotate("text", x = 12, y = 46, label = "SW = 0.45 * SL + 2.44",
             size = 3.5, color = "black", family = "serif")+
    annotate("text", x = 12, y = 41, label = paste("R^2 ==", round(S_SW_sum$r.squared, 2)), parse = TRUE,
             size = 3.5, color = "black", family = "serif"))
#
...
save_as_docx("Data summaries" = Data_summs,
             ...
             path = "Output/Age Size at Maturity_Tables_2023 05 25.docx", 
...
(Station_comps <- ggplot(StationLett, aes(Station, mean, fill = "black"))+
  ...
  scale_fill_grey()+
  geom_text(aes(label = Letters, y = mean+sd), size = 5, vjust = -0.35)+
  Base + theme(legend.position = "none", axis.title.y = element_blank()) +
  xlab("Habitat")+
...
Elefant %>% 
  ggplot(aes(SL))+
  geom_histogram(binwidth = 4, boundary = 0, closed = "left")+
  geom_text(data = Month_labels, aes(SL, count, label = Months), size = 3.5, family = "serif")+
...
#
#
ggsave(path = "Output/Figures/Final", filename = "D_Growth_Models.tiff", dpi = 1000, width = 8, height = 6, units = "in")
#
#
#
# 
...
(BFa_MR <- MR_test %>% ggplot(aes(End_SL, predSL))+
   geom_point()+
   geom_abline(intercept = 0, linetype = "dashed")+
   annotate("text", x = 12, y = 94, label = "RMSE = 1.63", family = "serif", size = 4)+
   scale_x_continuous("Observed shell length (mm)", expand = c(0,0), limits = c(0, 100))+
   scale_y_continuous("Predicted shell length (mm)", expand = c(0,0), limits = c(0, 100))+
   Base + theme(axis.title = element_blank()))
#
(W_MR <- MR_test_raw %>% ggplot(aes((L2*10), (predSL*10)))+
    geom_point()+
    geom_abline(intercept = 0, linetype = "dashed")+
    annotate("text", x = 12, y = 94, label = "RMSE = 3.40", family = "serif", size = 4)+
    scale_x_continuous("Observed shell length (mm)", expand = c(0,0), limits = c(0, 100))+
    scale_y_continuous("Predicted shell length (mm)", expand = c(0,0), limits = c(0, 100))+
    Base+ theme(axis.title = element_blank()))
#
...
(BFa_WL <- WL_test %>% ggplot(aes(End_SL, predSL))+
    geom_point()+
    geom_abline(intercept = 0, linetype = "dashed")+
    annotate("text", x = 12, y = 94, label = "RMSE = 4.28", family = "serif", size = 4)+
    scale_x_continuous("Observed shell length (mm)", expand = c(0,0), limits = c(0, 100))+
    scale_y_continuous("Predicted shell length (mm)", expand = c(0,0), limits = c(0, 100))+
    Base+ theme(axis.title.x = element_blank(), axis.title.y = element_text(hjust = -0.2)))
#
(W_WL <- WL_test_raw %>% ggplot(aes((L2*10), (predSL*10)))+
    geom_point()+
    geom_abline(intercept = 0, linetype = "dashed")+
    annotate("text", x = 12, y = 93, label = "RMSE = 7.50", family = "serif", size = 4)+
    scale_x_continuous("Observed shell length (mm)", expand = c(0,0), limits = c(0, 100))+
    scale_y_continuous("Predicted shell length (mm)", expand = c(0,0), limits = c(0, 100))+
    Base + theme(axis.title.y = element_blank()))
#
#
(RMSE_plots <- ggpubr::ggarrange(BFa_MR, W_MR, BFa_WL, W_WL, labels = c("A", "B", "C", "D"), 
                                    nrow = 4, ncol = 1, hjust = -6.1, vjust = 1.55, align = "v"))
#
#
ggsave(path = "Output/Figures/Final", filename = "F_Model_RMSE.tiff", dpi = 1000, width = 8, height = 6, units = "in")
#
#
#
...
(Treat_plots <- ggpubr::ggarrange(Treat_SL, Treat_Rate, labels = c("A", "B"), 
                                 nrow = 1, ncol = 2, hjust = -6.7, vjust = 1.55, align = "v"))
#
...
(Stage_plots <- ggpubr::ggarrange(Stages_MF, Stages_Month, labels = c("A", "B"), 
                                  nrow = 2, ncol = 1, hjust = -4.1, vjust = 0.75, align = "v", 
                                  common.legend = TRUE))
#
...
(Reg_plots <- ggpubr::ggarrange(S_WWwCI_eq, S_SWwCI_eq, labels = c("A", "B"), 
                                  nrow = 2, ncol = 1, hjust = -5.7, vjust = 1.25, align = "v", 
                                  common.legend = FALSE))
#
